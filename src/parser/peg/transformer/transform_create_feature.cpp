...
unique_ptr<CreateStatement> PEGTransformerFactory::TransformCreateFeatureStmt(PEGTransformer &transformer,
	...
	//                      FeatureEntityClause? 'TIMESTAMP' IdentifierOrStringLiteral
	//                      FeatureGranularityClause? FeatureWindowClause?
	...
	// index 5: FeatureEntityClause? (optional; empty = global feature)
	vector<string> entity_columns;
	auto &entity_opt = list_pr.Child<OptionalParseResult>(5);
	if (entity_opt.HasResult()) {
		// FeatureEntityClause <- 'ENTITY' FeatureEntitySpec
		auto &clause = entity_opt.GetResult().Cast<ListParseResult>();
		entity_columns = TransformFeatureEntitySpec(transformer, clause.Child<ListParseResult>(1));
	}
	// index 6: 'TIMESTAMP' keyword
	auto timestamp_column = transformer.Transform<QualifiedName>(list_pr.Child<ListParseResult>(7)).name;
	// index 8: FeatureGranularityClause? (default: DAY)
	FeatureGranularity granularity = FeatureGranularity::DAY;
	auto &granularity_opt = list_pr.Child<OptionalParseResult>(8);
	...
	// index 9: FeatureWindowClause? (default: 1)
	...
	// index 10: FeatureRefreshClause? (default: INCREMENTAL)
	FeatureRefreshMode refresh_mode = FeatureRefreshMode::INCREMENTAL;
	auto &refresh_opt = list_pr.Child<OptionalParseResult>(10);
	...
	info->entity_columns = std::move(entity_columns);
...
vector<string> PEGTransformerFactory::TransformFeatureEntitySpec(PEGTransformer &transformer,
                                                                 ParseResult &parse_result) {
	// FeatureEntitySpec <- Parens(List(IdentifierOrStringLiteral)) / IdentifierOrStringLiteral
	auto &list_pr = parse_result.Cast<ListParseResult>();
	auto &selected = list_pr.Child<ChoiceParseResult>(0).GetResult();

	vector<string> entity_columns;
	// The parenthesised-list branch begins with a '(' keyword token; the single-identifier branch
	// begins with a choice (Identifier / StringLiteral).
	if (selected.type == ParseResultType::LIST &&
	    selected.Cast<ListParseResult>().GetChild(0).type == ParseResultType::KEYWORD) {
		auto &inner = ExtractResultFromParens(selected);
		auto items = ExtractParseResultsFromList(inner);
		for (auto &item : items) {
			entity_columns.push_back(transformer.Transform<QualifiedName>(item).name);
		}
	} else {
		entity_columns.push_back(transformer.Transform<QualifiedName>(selected).name);
	}
...
unique_ptr<SQLStatement> PEGTransformerFactory::TransformServeFeatureStatement(PEGTransformer &transformer,
	...
	// index 5: optional ServeFeatureEntity <- 'ENTITY' FeatureEntitySpec
	// May be a single column or a parenthesised list (composite entity). Encoded as a comma-separated
	// string for the serve_feature() table function; empty signals "use the feature's own entity columns".
	string entity_columns;
	auto &entity_opt = list_pr.Child<OptionalParseResult>(5);
	if (entity_opt.HasResult()) {
		auto &entity_list = entity_opt.GetResult().Cast<ListParseResult>();
		// index 0: 'ENTITY' keyword, index 1: FeatureEntitySpec
		auto cols = TransformFeatureEntitySpec(transformer, entity_list.Child<ListParseResult>(1));
		for (auto &col : cols) {
	...
	// Rewrite to: CALL serve_feature('feature_names', 'spine_table', 'entity_columns', 'as_of_column')
	...
	args.push_back(make_uniq<ConstantExpression>(Value(entity_columns)));
